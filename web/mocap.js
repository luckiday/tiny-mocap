...
const currentChannel = (new Date()).getTime();
...
async function sendResult(landmarks) {
    let url = 'capture_motion';
    let data = {'id': currentChannel, 'time': (new Date()).getTime(), 'ldmk': landmarks};
...
function onResults(results) {
    // send the landmarks to the server
    let ldmks = [results.poseLandmarks, results.leftHandLandmarks, results.rightHandLandmarks];
    sendResult(ldmks).then(afterIkTransformation)
    ...
    drawingUtils.drawLandmarks(canvasCtx, results.leftHandLandmarks, {
        color: 'white', fillColor: 'rgb(255,138,0)', lineWidth: 2, radius: (data) => {
            return drawingUtils.lerp(data.from.z, -0.15, .1, 10, 1);
        }
    });
    // // Face...
...
new controls
    ...
    .add([
        ...
        // new controls.StaticText({title: 'Control Pad'}),
        // new controls.Slider({
        // title: 'Model Complexity', field: 'modelComplexity', discrete: ['Lite', 'Full', 'Heavy'],}),
        // new controls.Toggle({
        // title: 'Smooth Landmarks', field: 'smoothLandmarks'}),
        // new controls.Toggle({
        // title: 'Enable Segmentation', field: 'enableSegmentation'}),
        // new controls.Toggle({title: 'Smooth Segmentation', field: 'smoothSegmentation'}), new controls.Slider({
        // title: 'Min Detection Confidence', field: 'minDetectionConfidence', range: [0, 1], step: 0.01}),
        // new controls.Slider({
        // title: 'Min Tracking Confidence', field: 'minTrackingConfidence', range: [0, 1], step: 0.01}),
        // new controls.Slider({
        // title: 'Effect', field: 'effect', discrete: {'background': 'Background', 'mask': 'Foreground'},}),
    ])
    .on(x => {
        const options = x;
